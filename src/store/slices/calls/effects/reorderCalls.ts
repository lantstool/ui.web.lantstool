...
export const reorderCalls = effect(async ({ payload, slice, store }: any) => {
  const { currentOrder, newOrder } = payload;
  const [idb] = store.getEntities((store: any) => store.idb);
  const updateCallsPosition = slice.getActions((slice: any) => slice.updateCallsPosition);
  const list = store.getState((store: any) => store.calls.records);
  const { networkId, spaceId } = store.getState((store: any) => store.networks.current);
  ...
  try {
    ...
    await Promise.all(
      reorderList.map((id, index) => {
        const callToUpdate = calls.find((el: any) => el.callId === id);
        if (callToUpdate) {
