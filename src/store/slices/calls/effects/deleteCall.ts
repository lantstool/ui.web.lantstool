...
const updateCallsOrder = async (idb: any, txOrder: number, networkId: string) => {
  const tx = idb.transaction('calls', 'readwrite');
  const index = tx.store.index('spaceId_networkId_order');

  const updatedCallOrder: any = {};
...
const getNextRoute = (ids: any, activeTxId: string, networkId: string) => {
  // If we have only 1 tx in the list after we will have 0 records
  if (ids.length === 1) return `/${networkId}/calls`;
  const index = ids.findIndex((id: any) => id === activeTxId);
...
export const deleteCall = effect(async ({ payload, slice, store }: any) => {
  const { callId, navigate } = payload;
  const [idb] = store.getEntities((store: any) => store.idb);
  const removeCall = slice.getActions((slice: any) => slice.removeCall);
  const { order } = slice.getState((slice: any) => slice.records[callId]);
