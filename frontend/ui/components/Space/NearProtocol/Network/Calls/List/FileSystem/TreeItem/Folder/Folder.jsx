...
export const Folder = ({ item, wrapperProps, onCollapse, items }) => {
  ...
  const createOneCall = useStoreEffect((store) => store.nearProtocol.calls.createInFolder);
  ...
  const updateName = (formValues) =>
    updateOneName({ formValues, folderId: item.folderId, items, type: 'call' });

  const addCall = async () =>
    createOneCall({ spaceId, networkId, navigate, parentId: item.folderId, items });
    ...
    <>
      <div className={cnm(cn.folder, isActive && cn.activeFolder)} data-edit-id={item.folderId}>
        ...
        <div className={cn.settingsWrapper} onClick={(e) => e.stopPropagation()}>
          <button className={cn.addButton} onClick={addCall}>
